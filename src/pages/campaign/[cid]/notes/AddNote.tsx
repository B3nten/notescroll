...
export function AddNote() {
    ...
    async function addNote() {
        if (nameRef.current?.value.length > 3) {
            try {
                const { data, error } = await supabase.from('documents').insert({ name: nameRef.current.value, type: typeRef.current.value, campaign_id: router.query.cid }).single()
                    ...
                    <div className="flex flex-col items-start space-y-2">
                        ...
                        <label className='flex flex-col'>
                            ...
                            <select ref={typeRef} className='select select-primary'>
                                <option className='btn btn-sm' value='character'>Character</option>
                                <option className='btn btn-sm' value='location'>Location</option>
                                <option className='btn btn-sm' value='item'>Item</option>
                                <option className='btn btn-sm' value='lore'>Lore</option>
                                <option className='btn btn-sm' value='dream'>Dream</option>
                                <option className='btn btn-sm' value='other'>Other</option>
                            </select>
                        </label>
                    </div>
