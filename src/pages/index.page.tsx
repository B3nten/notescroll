...
export default function Index() {
  ...
  return (
    <>
      {session.session && <div className='relative p-4 w-full mb-2 flex justify-end items-center space-x-2'>
      ...
      <h1 className='text-3xl font-heading text-center'>Hello {session.session.user.user_metadata.user_name}</h1>
            ...
            <Loading loaded={[campaigns.loaded]}>
              <ul className='text-lg space-y-2'>
                {campaigns.data?.map((c: any) =>
                  <li key={c.id} className='flex items-center justify-between p-2 bg-base-300 rounded-md'>
                    <div>{c.name}</div>
                    <button onClick={() => { router.push('/campaign/' + c.campaign_id) }} className='btn btn-sm'>Open</button>
                  </li>)}
              </ul>
            </Loading>
            <Dialog open={newCampaignModal}>
              <DialogTrigger asChild>
                <button onClick={() => setNewCampaignModal(true)} className='btn btn-circle mb-0 mx-auto'>+</button>
              </DialogTrigger>
            ...
            <Loading loaded={[recentDocs.loaded]}>
              <ul className='text-lg space-y-2'>
                {recentDocs.data?.map((c: any) =>
                  <li key={c.id} className='flex items-center justify-between p-2 bg-base-300 rounded-md'>
                    <div>{c.name}<span className='text-sm opacity-70'>{' '}{c.type}</span></div>
                    <button onClick={() => { router.push('/campaign?id=' + c.campaign_id) }} className='btn  btn-sm'>Open</button>
                  </li>)}
              </ul>
            </Loading>
          </div>
...
interface CreateCampaign { close: any }
function CreateCampaign({ close }: CreateCampaign) {
  ...
  async function handleClick() {
    if (name.current.value.length > 3 && startDate.current?.value) {
      ...
      showLoading(true)
      try {
        const { error } = await supabase.from('campaigns').insert({ name: name.current.value, startdate: startDate.current.valueAsNumber, setting: setting.current.value })
